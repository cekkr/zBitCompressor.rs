...
use crate::{
    ...
    estimator::preflate_parameter_estimator::{
        TokenPredictorParameters, estimate_preflate_parameter_candidates,
    },
...
impl PreflateStreamProcessor {
    ...
    pub fn decompress(&mut self, compressed_data: &[u8]) -> Result<PreflateStreamChunkResult> {
        ...
        if let Some(predictor) = &mut self.predictor {
            ...
            let mut params_candidates =
                estimate_preflate_parameter_candidates(&contents, &self.parser.plain_text())
                    .context()?;
            params_candidates.retain(|params| params.max_chain <= self.max_chain_length);
            if params_candidates.is_empty() {
                return Err(PreflateError::new(
                    ExitCode::NoCompressionCandidates,
                    format!(
                        "all candidates exceed configured max_chain_length {}",
                        self.max_chain_length
                    ),
                ));
            }

            let mut best: Option<(TokenPredictorParameters, TokenPredictor, Vec<u8>)> = None;
            for params in params_candidates {
                let mut candidate_corrections = Vec::new();
                let mut candidate_encoder =
                    PredictionEncoderCabac::new(VP8Writer::new(&mut candidate_corrections).unwrap());
                let mut candidate_predictor = TokenPredictor::new(&params);
                let mut candidate_input = PreflateInput::new(&self.parser.plain_text());
                ...
                let candidate_blob = bitcode::encode(&ReconstructionData {
                    parameters: params,
                    corrections: candidate_corrections,
                });
                let candidate_size = candidate_blob.len();

                match &best {
                    Some((_, _, best_blob)) if best_blob.len() <= candidate_size => {}
                    _ => {
                        best = Some((params, candidate_predictor, candidate_blob));
                    }
                }
            }

            let (params, token_predictor, reconstruction_data) = best.ok_or_else(|| {
                PreflateError::new(
                    ExitCode::NoCompressionCandidates,
                    "failed to evaluate any predictor candidate",
                )
            })?;
