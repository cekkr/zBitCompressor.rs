...
use super::{
    ...
    complevel_estimator::{estimate_preflate_comp_level, estimate_preflate_comp_level_candidates},
    preflate_stream_info::{PreflateStreamInfo, extract_preflate_info},
};

fn build_no_dictionary_params(
    preflate_strategy: PreflateStrategy,
    huff_strategy: BlockTypeStrategy,
...
fn build_params_from_comp_level(
    info: &PreflateStreamInfo,
    add_policy: DictionaryAddPolicy,
    window_bits: u32,
    max_token_count: u16,
    huff_strategy: BlockTypeStrategy,
    cl: super::complevel_estimator::CompLevelInfo,
) -> TokenPredictorParameters {
...
pub fn estimate_preflate_parameter_candidates(
    deflate_contents: &DeflateContents,
    plain_text: &PlainText,
) -> Result<Vec<TokenPredictorParameters>> {
    ...
    {
        return Ok(vec![build_no_dictionary_params(preflate_strategy, huff_strategy)]);
    }
    ...
    let comp_candidates = estimate_preflate_comp_level_candidates(
        window_bits,
        mem_level,
        info.min_len,
        deflate_contents,
    ...
    let mut out = Vec::with_capacity(comp_candidates.len());
    for cl in comp_candidates {
        let params = build_params_from_comp_level(
            &info,
            add_policy,
            window_bits,
            max_token_count,
...
pub fn estimate_preflate_parameters(
    ...
    {
        // No dictionary used
        return Ok(build_no_dictionary_params(preflate_strategy, huff_strategy));
    }
    ...
    Ok(build_params_from_comp_level(
        &info,
        add_policy,
        window_bits,
        max_token_count,
